...
| `implementation-readiness.md` | A decision-oriented readiness & phasing assessment. | Before funding a build — open questions, must/should/nice services, a day-one capability map (team self-service + the security "say-yes" surface), market-first phasing, the tech-stack roles, a tiered test strategy (LocalStack vs. real AWS), AWS cost (~$0.5–2.5k/mo) and the guardrails that bound it. |
