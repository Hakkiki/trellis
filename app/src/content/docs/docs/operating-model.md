...
This is the companion to [Why Trellis](/trellis/docs/the-case). That page argues for per-division
environments; this one covers the how: where the boundary goes, why synchronous-only coupling is the
...
But "we don't care where the boundary is" holds only for the *abstraction*. Operationally you care a
great deal, because:
...
- **Default: account-per-division**, grouped into OUs by domain. You get strong isolation (separate IAM,
  quotas, blast radius) and you keep the central lever (SCPs + delegated admin + the approved-services
  whitelist).
- **Separate org**: reserve for the highest-isolation cases, like strict regulatory separation or
  a real M&A / divestiture boundary. It's the strongest wall, but it gives up easy central governance.
...
reconciler**. It runs the same closed loop Trellis runs, with declared state in etcd, controllers driving
reality to match, and its own RBAC and god-write. So this is a reconciler managing a reconciler, and whether
...
wrong and you hit the classic failure of every "manage k8s from outside" tool, where two reconcilers
fight over the same object, each undoing the other. The rule that prevents it: Trellis stops at the
cluster API and platform add-ons, in-cluster GitOps (Argo/Flux) owns workloads, and they never overlap.
...
- **Upgrades are where "infra only" blurs.** Kubernetes removes APIs every minor version, so a cluster
  ...
  deprecated-API check, which pokes into workload territory. It's the one genuinely messy seam, so budget
...
So: Trellis manages the cluster, Kubernetes manages what's in it. Slice at the cluster,
...
If divisions talk to each other only **synchronously**, via request/response, with no shared database and no shared
...
(route + port) that crosses the containment boundary by design, and Trellis provisions and heals those edges.
...
Upgrading one division's control plane, canaried, dual-control, and recoverable, cannot touch another's.
...
None of these is a live god-write service that can take everyone down. They're
...
So even the shared catalog is a pull-and-promote registry, not a synchronous-fate
...
- **Per-division Trellis + shared catalog/governance** is the sweet spot. The control loop is contained;
  only the signed registry and the SCP rails are shared.
- **One central Trellis** is convenient, but you've rebuilt the SPOF. Avoid it for the same reason you
  avoid namespaces.
- **Per-division Trellis + its own catalog fork** gives maximum isolation but loses central governance. Reserve
  it for separate-org / M&A / strict-regulatory cases.

## The tradeoffs

- **N control planes is N things to run**, but they're cheap because Trellis is self-managing (a C0
...
and pulled. That's the operating model. The [specification](/trellis/docs/spec) is the source of truth.
