...
This page extends and applies the [specification](/trellis/docs/spec), which is the source of truth; it is not itself normative. For what the engine actually builds today versus what it doesn't, see [Status](#status-what-the-engine-implements).
...
off, and that drift is exactly what "we optimized your spend 30%" quietly becomes.
...
Closing this gap is a model change, not a tuning pass. Cost-effectiveness becomes a reconciled
...
Idle capacity comes in two flavours, so there are two levers. They deliberately share the same three-tier
...
So `sensitive === false` falls in the elasticity class and `sensitive === true` falls in the dormancy class. Every kind the
...
holds the full contract). This section says which lever owns which
...
> Tier is a *default keyed off environment*, but the variable that matters is duty cycle, not env name. A
...
2. **Parity is an enforced invariant.** The desired *shape* (replicas, regions, resilience)
...
- A temporal demand model (axis 1, declared half). `autoscale()` is no longer blind to time: it reads
  ...
  neighbours), pre-warms ahead of predicted demand, and refuses to park into it, so the DB
...
to the owner as a proposal: the machine proposes from observed reality and the owner ratifies at the gate,
...
2. **Low utilization → right-size (the *how much*), built as a gate-ratified proposal.** A resource
   ...
   shrink the average would wrongly suggest). It only proposes (`snapshot().rightSizing`):
...
Across all three the control action is to regulate, holding cost-against-value in band as demand,
prices, and business drivers move, rather than re-running an optimizer on a timer (which would chase the
