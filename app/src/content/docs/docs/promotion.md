...
The base desired state is **environment-blind**; each environment instantiates it with its own
**posture overrides** (the cascade — dev = C3, prod = C0). You promote a known-good artifact (vN), so
**what you validated in staging is bit-for-bit what reaches prod.**

## The one key split

The **artifact is promoted, but the *path* is re-planned per environment.** Prod gets a fresh transition
plan against *prod's* observed state, because prod ≠ staging. Promotion state is visible
...
- **Promote** it to `staging` (C2, active-passive) and `prod` (C0, active-active) — each re-plans for its
  own criticality and converges against its own simulated cloud.
