...
We build the implementation on AWS: one provider, executed richly. The vocabulary, Topology, and Structure stay **provider-neutral**, expressed against a capability contract. This page is the escape hatch. It maps that contract to AWS (the column we build) and writes down the GCP and Azure equivalents, so adding a provider later is an additive, parity-gated adapter rather than a rewrite. The [specification](/trellis/docs/spec) is the source of truth.

> **The rule (spec §15):** one provider executes at a time. Implement the AWS column; the rest is the documented crosswalk, built on demand. This mitigates provider risk; it is not active multi-cloud.

A capability with no clean equivalent is **lock-in exposure**. We call it out in the Notes column rather than pretend the abstraction is leak-free.
...
The contract is clean for most buckets. A few seams would cost real work to port:
...
- **Now:** we build the AWS column richly and ship, with no multi-cloud tax and no
...
- **Later:** adding GCP or Azure is a new adapter against the same capability contract, parity-gated
  against AWS. It is additive, not a rewrite. This page is the starting crosswalk for that adapter.
